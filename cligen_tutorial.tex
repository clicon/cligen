...
There are situations when a command matches multiple variables. For
this purpose, there is variable {\em preference} to resolve
tiebreaks. 
...
If a user types the command ``mycommand'', it matches all three rules, ``foo''
matches the two variables and ``foo42'' matches only the last.
...
This means that typing ``mycommand'' matches the command, ``foo''
matches the regexp and ``foo42'' matches the string type.

The default variable preference table is available in Appendix~D.

\subsubsection{Resolving tie-breaks}

Even with variable preferences, there may be several matching
variables with same preference. For example strings with regular expressions may overlap:
\begin{verbatim}
  <name:string regexp:"[a-b]+">;
  <name:string regexp:"[b-c]+">;
\end{verbatim}
where the string ``bbb'' matches both rules with equal preference, which results in ``Ambiguous command'' in the CLI.

There are two methods in CLIgen to break this tie-break:
\begin{enumerate}
\item {\tt preference} keyword
\item {\tt cligen\_preference\_mode\_set()} API function
\end{enumerate}

The {\tt preference} keyword alters the preference of a variable, such as follows:
\begin{verbatim}
...
Using the {\tt cligen\_preference\_mode()} API, a tie-break can be
resolved by choosing the {\em first} matching command for {\em all}
preference tie-breaks in a system. This may be useful if one knows
that this does not pose a problem.

However, you need to be careful if effects of the commands are different. For example, in a syntax such as:
\begin{verbatim}
  <name:string regexp:"[a-b]+">, A();
  <name:string regexp:"[b-c]+">, B();
\end{verbatim}

it may be dangerous to use the API method, if A() and B() do different things.

But if all cases of tie-breaks are on the following form:
...
Differentiation between terminals and non-terminals menas tie-breaks in the terminating command (such as: {\tt foo (<var>|<var>);} or an intermediate such as: {\tt foo (<var>|<var>) command; }

The settings are:

\begin{enumerate}
\item Terminals, ie the 
\item Non-terminals
