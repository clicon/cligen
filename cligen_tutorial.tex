...
% Title
%
\title {CLIgen Manual\\ {\small CLIgen version 3.7}}
...
\begin{verbatim}
  <int32>;                
  <a:int32>;              
  <a:int32 show:number>("A 32-bit number")
\end{verbatim}
...
A string can be constrained by a {\tt length} statement. If given, the number of characters in a string is limited to a min/max interval, or just a max.
...
\end{verbatim}
which means that the {\tt addr} string, if given, must be between 8 and 12 characters long, or just limited to 12 characters, respectively.
...
Further, an allowed range of integer can specified, either as an interval or as an upper limit.
...
In the same way, a clear text password could e translated to an encrypted string.
...
\begin{verbatim}
  ...
  # $Id: cligen_tutorial.tex,v 1.33 2014/11/09 19:58:30 olof Exp $
  ...
  ip("The IP protocol"){       
    tcp("The TCP protocol") <uint16>("Port number"), cb("tcp");
  ...
  values (<int8> | <string> | <int64> | aa), cb();
...
\emph{  Delete a cv variable from a cvec. Note: cv is not reset and may be stale!}
...
\emph{  Assign functions for callbacks in a parse-tree using a translate function}
...
{\tt int8\_t cv\_int8\_get(cg\_var *cv)}\\*
\emph{ Get 8-bit integer value of cv}

{\tt int16\_t cv\_int16\_get(cg\_var *cv)}\\*
\emph{ Get 16-bit integer value of cv}

{\tt int32\_t cv\_int32\_get(cg\_var *cv)}\\*
\emph{ Get 32-bit integer value of cv}

{\tt int64\_t cv\_int64\_get(cg\_var *cv)}\\*
\emph{ Get 64-bit integer value of cv}

{\tt uint8\_t cv\_uint8\_get(cg\_var *cv)}\\*
\emph{ Get 8-bit unsigned integer value of cv}

{\tt uint16\_t cv\_uint16\_get(cg\_var *cv)}\\*
\emph{ Get 16-bit unsigned  integer value of cv}

{\tt uint32\_t cv\_uint32\_get(cg\_var *cv)}\\*
\emph{ Get 32-bit unsigned integer value of cv}

{\tt uint64\_t cv\_uint64\_get(cg\_var *cv)}\\*
\emph{ Get 64-bit unsigned integer value of cv}
...
{\tt char *cv\_flag(cg\_var *cv)}\\*
\emph{  Get application-specific cv flag. }

{\tt char *cv\_flag\_clr(cg\_var *cv)}\\*
\emph{  Clear application-specific cv flag. }

{\tt char *cv\_flag\_set(cg\_var *cv)}\\*
\emph{  Set application-specific cv flag. }
