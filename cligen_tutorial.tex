...
% Title
%
\title {CLIgen Manual\\ {\small CLIgen version 5.2}}
...
\item
{\tt hide-database} : specifies that a command is not visible in database. This can be useful for setting passwords and not exposing them to users.
\item
{\tt hide-database-auto-completion} : specifies that a command is not visible in database and in auto completion. This can be useful for a password that was put in device by super user, not be changed.
\end{itemize}
...
Note, recursive tree references are not possible if {\tt
  reftree\_copy} is $0$, which is the case for Clixon and many uses of
CLIgen. However, {\tt reftree\_copy} is $1$ by default, which means
recursive tree-refs work in plain CLIgen.
...
It is possible to pre-set a set of labels that are filtered by default using {\tt cligen\_reftree\_filter\_set}, in which you can enable a label in a reference using eg {\tt @T, @add:local;}.
...
Two API functions control the format of {\tt cvv} and changes the rules above:
\begin{enumerate}
